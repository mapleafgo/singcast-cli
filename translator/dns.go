...
func translateDNS(cfg *RawConfig, t *translation) {
	...
	for pattern, serverVal := range dns.NameServerPolicy {
		...
		rule := nameserverPolicyToRule(pattern, policyTag, t)
		if rule != nil {
...
func nameserverPolicyToRule(pattern string, serverTag string, t *translation) map[string]any {
	// Skip geosite: patterns — DNS rules are generated by geo-based auto-routing instead.
	...
	if strings.HasPrefix(pattern, "+.") {
		return map[string]any{
			"domain_suffix": []string{pattern[1:]},
			"server":        serverTag,
		}
	}
